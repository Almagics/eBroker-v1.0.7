...
class FetchOutdoorFacilityListCubit
  ...
  void fetch() async {
    try {
      ...
      emit(FetchOutdoorFacilityListFailure(error: e));
    }
  }

  List<OutdoorFacility> getList() {
    log(state.toString(), name: "STATEIS");
    if (state is FetchOutdoorFacilityListSucess) {
      log("List is ${state} state");
      log("LISSSTT IS  ${(state as FetchOutdoorFacilityListSucess).outdoorFacilityList.map((e) => e.name).toList().toString()}");
